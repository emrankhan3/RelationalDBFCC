...
echo "$($PSQL "SELECT distinct(course) from students right join majors using (major_id) inner join majors_courses using (major_id) inner join courses using (course_id) where first_name = 'Obie' and last_name ='Hilpert' or students.major_id is null order by course desc")"
...
echo "$($PSQL "SELECT course from courses left join majors_courses using (course_id) inner join majors using (major_id) inner join students using (major_id) group by course  having count(*)=1 order by course")"
